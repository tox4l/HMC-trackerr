      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-8">
            <Reveal delay={0.3}>
              <GlassCard className="bg-gradient-to-br from-white/10 to-transparent border-white/10">
                <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
                  <Activity className="h-5 w-5 text-emerald-400" />
                  Maintenance Status as of {now.toLocaleDateString()}
                </h3>

                <div className="space-y-6">
                  <div className="relative pl-4 border-l-2 border-white/10 space-y-1">
                    <span className="text-xs text-slate-400 uppercase tracking-wider">Last Service</span>
                    <div className="text-xl font-bold text-white">
                      {equipment.lastMaintenanceDate?.toLocaleDateString() || 'Never'}
                    </div>
                    <div className="text-sm text-slate-500">
                      {equipment.lastMaintenanceDate ?
                        `${Math.floor((now.getTime() - equipment.lastMaintenanceDate.getTime()) / (1000 * 60 * 60 * 24))} days ago` :
                        'No record'}
                    </div>
                  </div>

                  <div className={`relative pl-4 border-l-2 ${isOverdue ? 'border-red-500' : 'border-blue-500'} space-y-1`}>
                    <span className="text-xs text-slate-400 uppercase tracking-wider">Next Due</span>
                    <div className={`text-xl font-bold ${isOverdue ? 'text-red-400' : 'text-blue-400'}`}>
                      {equipment.nextMaintenanceDate.toLocaleDateString()}
                    </div>
                    <div className={`text-sm ${isOverdue ? 'text-red-400 font-medium' : 'text-blue-300'}`}>
                      {isOverdue ? `${Math.abs(daysUntilDue)} days overdue` : `in ${daysUntilDue} days`}
                    </div>
                  </div>

                  <div className="relative pl-4 border-l-2 border-emerald-500 space-y-1">
                    <span className="text-xs text-slate-400 uppercase tracking-wider">Est. Cost</span>
                    <div className="text-xl font-bold text-emerald-400">
                      QAR {estimatedCost.toLocaleString()}
                    </div>
                ...
                <div className="mt-8 pt-6 border-t border-white/10">
                  <button className="w-full py-3 bg-white text-black font-semibold rounded-xl hover:bg-slate-200 transition-colors flex items-center justify-center gap-2">
